...
typedef enum {
    mInstallAttributesMask=0x01,
    mInstallActionsMask=0x01<<1,
    mInstallTypeMask=~0x000<<2,
} mInstallFlagMasks;

typedef enum {
    bInstallAttributes=0x01,
    bInstallActions=0x02,
    //Already shifted....
    bInstallMerge=0x04,
    bInstallCross=0x08,
    bInstallExclude=0x0c,
    bInstallOneNight=0x10
} bInstallConstants;


@interface RE_Entity : NSObject
{
...
//    RE_ObservableDict * replacelater;
    RE_ObservableDict * _attributes;
    
    // Use this to maintain slots of equipments.
    // But we don't want to differentiate equipments and items explicitly...
    // Cause the slots implementation hurts the abstraction, which add 
    // some more fields as slotname etc. to the RE_ENtity class.
    // So here we just use slotname as the key indexing into _equips,
    // while the RE_Entity preserver a name in itself.
    // And we have to modify the InstallEntity API to name the name explicitly.
    // And so forth the one_night flag turns out to be tedious.
    
    // Further more the slotname may be used as filters when do the equipment
    // browsing.
    // In conclusion, a Subclassing operation for the slotname field is 
    // INEVITABLE.
    NSMutableDictionary* _equips;
    
    NSString* _name;
	RE_Actor * _curSprite ;
    /*  |0   |1  |2 - 4                           |
        |attr|act|merge/cross/exclusive/one_night |
     */
    int _install_flags;
    int _bindState;    
...
@property (atomic, readwrite) int installFlag;
@property (assign, readonly) NSDictionary* attributes;
@property (assign, readonly) NSDictionary* actions;
@property (retain, readwrite) NSString* name;
@property (retain, readwrite) RE_Actor* sprite;

// Temporery constructor for plist attribs and already made action functions.
-(id) initWithAttributes:(NSDictionary*)attrs 
             withActions:(NSDictionary*)acts;

-(void) handleMessage:(RE_Msg*)message;
//"""the RPC api for all calls on a game object"""
//self.main_handler(message)

-(void) sendMessage:(RE_Msg*)message to:(RE_Entity*)target;
//self.main_handler.sendMessage(message)	
-(void) reply:(RE_Msg*)message withAction:(RE_Action*)action;

//-(void) takeActionWithName:(NSString*) action;
-(RE_Action*) performAction:(NSString*) act_name;
//-(void) takeAction:(RE_Action*)action;
//-(RE_Action*) makeAction:(RE_Action*)act;
-(RE_Action*) runAction:(RE_Action*)action;
-(void) installAction:(NSString*) action;
-(void) uninstallAction:(NSString*) name;
-(RE_Action*) getAction:(NSString*)name;

-(BOOL) isAttributeLocked:(NSString*)name;
-(BOOL) setAttribute:(float)value withName:(NSString*)name;
-(float) getAttribute:(NSString*)name;

-(void) installAttribute:(RE_CharacterAttribute*) attribute named:(NSString*)name;
-(void) uninstallAttribute:(NSString*)name;

-(void) installEntityAttrib:(RE_Entity*) entity;
-(void) uninstallEntityAttrib:(RE_Entity*)entity;
-(void) installEntityActions:(RE_Entity*)entity;
-(void) uninstallEntityActions:(RE_Entity*)entity;

// Trending to desperate.
-(void) InstallEntity:(RE_Entity*)entity;
-(void) UninstallEntity:(RE_Entity*)entity_index;

-(RE_Entity*) InstallEntity:(RE_Entity*)entity withName:(NSString*)name;
-(RE_Entity*) UninstallEntityWithName:(NSString*)name;

-(void) installSprite:(RE_Actor*)sprite;
-(void) messageMap:(RE_Msg*)message;
...
@interface RE_EntityGroup : NSObject {

    NSMutableDictionary* _sprites;
    NSMutableDictionary* _buddies;
    NSMutableDictionary* _items;
    NSMutableDictionary* _items_compile;
    RE_Entity* _leader;
}

//@property (retain, readwrite) NSMutableDictionary*sprites;
//@property (retain, readwrite) NSMutableDictionary*buddies;
//@property (retain, readwrite) NSMutableDictionary*items;
//@property (retain, readwrite) NSMutableDictionary*items_compile;
//@property (retain, readwrite) RE_Entity*leader;
